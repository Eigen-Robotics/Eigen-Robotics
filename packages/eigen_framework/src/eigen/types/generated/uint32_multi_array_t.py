...
class uint32_multi_array_t(object):
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(uint32_multi_array_t._get_packed_fingerprint())
    ...
    def decode(data: bytes):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != uint32_multi_array_t._get_packed_fingerprint():
            raise ValueError("Decode error")
        return uint32_multi_array_t._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        self = uint32_multi_array_t()
    ...
    def _get_hash_recursive(parents):
        if uint32_multi_array_t in parents: return 0
        newparents = parents + [uint32_multi_array_t]
    ...
    def _get_packed_fingerprint():
        if uint32_multi_array_t._packed_fingerprint is None:
            uint32_multi_array_t._packed_fingerprint = struct.pack(">Q", uint32_multi_array_t._get_hash_recursive([]))
        return uint32_multi_array_t._packed_fingerprint

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", uint32_multi_array_t._get_packed_fingerprint())[0]
