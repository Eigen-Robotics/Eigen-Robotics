...
from ark.sim.pybullet.pybullet_robot_driver import BulletRobotDriver


@_register_default_component(
    ComponentSpec(
        component_type=ComponentType.ROBOT, id="franka-base", is_driver=True
    )
)
class FrankaPyBulletDriver(BulletRobotDriver):
    """Franka PyBullet driver for simulation.

    Attributes:
        _runnable: Always True since PyBullet is a standard dependency.
    """
