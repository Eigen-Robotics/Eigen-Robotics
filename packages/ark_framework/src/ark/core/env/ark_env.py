...
from ark.core.client.comm_infrastructure.instance_node import InstanceNode
from ark.core.env.spaces import ActionSpace, ObservationSpace
from ark.core.tools.log import log
from ark.types import flag_t, rigid_body_state_t, robot_init_t
...
class ArkEnv(Env, InstanceNode, ABC):
    ...
    def __init__(
        ...
        # TODO(FV): Define a global type alias for the config or ensure it is only Path
        global_config: str | dict[str, Any] | Path = None,
        sim=True,
    ) -> None:
        ...
        self._multi_comm_handlers.append(
            self.action_space.action_space_publisher
        )
        self._multi_comm_handlers.append(
    ...
    def reset_component(self, name: str, **kwargs):
        ...
        if name in self.global_config["robots"]:
            ...
            request.position = kwargs.get(
                "base_position",
                self.global_config["robots"][name]["base_position"],
            )
        ...
        # elif name in [obj["name"] for obj in self.global_config["objects"]]:
            ...
            request.position = kwargs.get(
                "base_position",
                self.global_config["objects"][name]["base_position"],
            )
            request.orientation = kwargs.get(
                "base_orientation",
            ...
            # TODO(PREV) for now we only work with position init, may add velocity in the future
            request.lin_velocity = kwargs.get("base_velocity", [0.0, 0.0, 0.0])
            request.ang_velocity = kwargs.get(
    ...
    def _load_config(self, global_config) -> None:
        ...
        config["robots"] = (
            self._load_section(cfg, config_path, "robots")
            if cfg.get("robots")
            else {}
        )
        config["sensors"] = (
