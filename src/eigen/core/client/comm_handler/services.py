...
class Service(CommHandler):
    ...
    def __init__(
        ...
        self.channel_request_type = req_type
        self.channel_response_type = resp_type
    ...
    def register_with_registry(self):
        ...
        registration = {
            "type": "REGISTER",
            "service_name": self.channel_name,
            ...
            while not self._stop_event.is_set():
                ...
                with conn:
                    try:
                        ...
                        # Process the request
                        response = self.callback(self.channel_name, request)
                        # Encode the response
    ...
    def __repr__(self):
        ...
        @return: A string representation of the handler, formatted as
                "channel_name[request_type,response_type]".
        """
        return f"{self.channel_name}[{self.req_type},{self.resp_type}]"
    ...
    def restart(self):
        ...
        """
        if self.active:
            log.debug("service %s already active; restart skipped", self.channel_name)
            return
        self._stop_event = threading.Event()
        self.thread = self._spawn_thread()
        self.registered = self.register_with_registry()
        self.active = bool(self.registered)
        if not self.active:
            log.error("Service '%s' failed to register during restart.", self.channel_name)
    ...
    def suspend(self):
        ...
        if not self.active:
            return
        deregistered = self.deregister_from_registry()
        self._stop_event.set()
        self.thread.join()
        self.active = False
        self.registered = False
        if deregistered:
            log.info("Service '%s' stopped.", self.channel_name)
        else:
            log.error("Service '%s' shutdown un-gracefully.", self.channel_name)
    ...
    def deregister_from_registry(self) -> bool:
        ...
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                s.sendall(encoded_req)
                log.info(
                    f"Service: Sending deregistration request for '{self.channel_name}'."
                )
                ...
                if response.get("status") == "OK":
                    log.info(
                        f"Service: Successfully deregistered '{self.channel_name}' from registry."
                    )
                    return True
                else:
                    log.error(
