...
class GenesisRobotDriver(SimRobotDriver):
    ...
    def load_robot(
        ...
        fixed_base: bool = self.config.get("use_fixed_base", 1)
        self.ref_body_id = self.client.add_entity(
            gs.morphs.URDF(file=str(urdf_path),
                           fixed=fixed_base,
                           pos=list(base_position),
                           quat=list(base_orientation),
                           )
        )

        log.ok(
            f"Initialized robot specified by URDF '{mjcf_path}' in Genesis simulator."
        )
